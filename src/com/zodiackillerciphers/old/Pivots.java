...
public class Pivots {
	...
	public static List<Pivot> pivots(String sub, int minLegLength) {
		String[] d = {"N","E","S","W"};
	...
	public static void pivotSearch(int minLegLength) {
		...
		for (int i=0; i<line.length()-L; i+=L) {
			...
			// locate any pivots
			List<Pivot> pivots = pivots(sub, minLegLength);
...
//				dump(sub, pivots);
	...
	public static int count(List<Pivot> pivots, int minLegLength, boolean sameOrientation, String sub) {
		int c = 0;
		String maxPair = null;
		if (sameOrientation) {
	...
	public static void testPivots() {
		...
		List<Pivot> list = pivots(sub, 3);
		for (Pivot p : list) if (p.longestLeg() > 0) System.out.println(p);
	}
