...
public class ZodiacBreeder extends SimpleBreeder {
  ...
  public boolean lt(long a, long b) {
  	...
  	float[] fitA = ((MultiObjectiveFitness)geneA.fitness).multifitness;
  	float[] fitB = ((MultiObjectiveFitness)geneB.fitness).multifitness;
  ...
  public boolean gt(long a, long b) {
	...
	public void loadElites(EvolutionState state, Population newpop) {
		...
		for (int sub = 0; sub < state.population.subpops.length; sub++) {
			...
			int maxFit = (int)((MultiObjectiveFitness)oldinds[orderedPop[inds.length-1]].fitness).multifitness[0];
			...
			for (int i=inds.length-1; i>=0; i--) { // stick every gene in a bin based on its first objective.
				fit = (int)((MultiObjectiveFitness)oldinds[orderedPop[i]].fitness).multifitness[0];
